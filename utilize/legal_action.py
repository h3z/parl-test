...
def check_gen_p(adjust_gen_p, action_space_gen_p, gen_ids, eps):
    illegal_gen_ids = [i for i in gen_ids if
                       adjust_gen_p[i] < action_space_gen_p.low[i] - eps or adjust_gen_p[i] > action_space_gen_p.high[
                           i] + eps]
...
def check_gen_v(adjust_gen_v, action_space_gen_v, gen_ids, eps):
    illegal_gen_ids = [i for i in gen_ids if
                       adjust_gen_v[i] < action_space_gen_v.low[i] - eps or adjust_gen_v[i] > action_space_gen_v.high[
                           i] + eps]
    return illegal_gen_ids


def check_adjld_p(adjust_adjld_p, action_space_adjld_p, adjld_ids, eps):
    illegal_adjld_ids = [i for i in adjld_ids if
                         adjust_adjld_p[i] < action_space_adjld_p.low[i] - eps or adjust_adjld_p[i] >
                         action_space_adjld_p.high[i] + eps]
    return illegal_adjld_ids


def check_stoenergy_p(adjust_stoenergy_p, action_space_stoenergy_p, stoenergy_ids, eps):
    illegal_stoenergy_ids = [i for i in stoenergy_ids if
                             adjust_stoenergy_p[i] < action_space_stoenergy_p.low[i] - eps or adjust_stoenergy_p[i] >
                             action_space_stoenergy_p.high[i] + eps]
...
def is_legal(act, last_obs, settings):
    ...
    if stoenergy_p_illegal_ids:
        illegal_reasons.append(
            StoenergyPOutOfActionSpace(stoenergy_p_illegal_ids, action_space_stoenergy_p, adjust_stoenergy_p))
